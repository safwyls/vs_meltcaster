...
{
    public class MeltcasterConfigOld
    {
        ...
        public List<MeltcastRecipeOld>? MeltcastRecipes { get; set; }

        [JsonIgnore]
        public Dictionary<string, MeltcastRecipeOld>? MeltcastRecipeByCode { get; private set; }
        ...
        public static MeltcasterConfigOld GetDefault()
        {
            return new MeltcasterConfigOld
            {
                MeltcastRecipes = new List<MeltcastRecipeOld>
                {
                    new MeltcastRecipeOld
                    {
                        ...
                        MeltcastTemp = 1100,
                        MeltcastTime = 30,
                        Outputs = new List<MeltcastOutputOld>
                        {
                            new()
                            {
                                IsGroup = true,
    ...
    public class MeltcastRecipeOld
    {
        [JsonProperty("input")]
        ...
        public List<MeltcastOutputOld>? Outputs { get; set; }
    ...
    public class MeltcastOutputOld
    {
        ...
        public List<MeltcastOutputOld>? ItemGroup { get; set; }

        [JsonIgnore]
        public JsonItemStack? ResolvedItem => IsGroup ? null : ItemStack;

        public MeltcastOutputOld? WeightedRandomOrFirst(ICoreAPI api)
        {
            if (ItemGroup != null && ItemGroup.Count > 0)
            {
                ...
                float accum = 0f;
                foreach (MeltcastOutputOld option in ItemGroup)
                {
                    accum += option.Chance;
                    if (accum >= random)
                    {
